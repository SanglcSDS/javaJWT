$(document).ready(function(){
	...
	$(document).on('submit', '#formCreateDevice', function(e){
		...
		$.ajax({
			...
			url: getContextPath() + "/device/save",
			data: JSON.stringify({
				name: $('#name').val(),
			}),
			cache: false,
	...
	function getList(){
		$.ajax({
			type: "GET",
			contentType: "application/json",
			url: getContextPath() + "/device/list",
			success: function(result){
				let stt = 0;
				$('#tblDevice').dataTable({
					"lengthChange": true,
	...
	$(document).on('click', '.btn-delete', function(){
		...
		Swal.fire({
		  ...
		  if (result.isConfirmed) {
			 $.ajax({
                url: getContextPath() + '/device/delete',
                type: 'POST',
				contentType: "application/json",
                data: JSON.stringify({
					id: id
	...
	$(document).on('click', '.btn-edit', function(){
		let id = $(this).attr('data-id');
		$.ajax({
            url: getContextPath() + '/device/edit',
            type: 'POST',
			contentType: "application/json",
            data: JSON.stringify({
				id: id
	...
	$(document).on('submit', '#formUpdateDevice', function(e){
		...
		if(check){
			$.ajax({
				type: "POST",
				contentType: "application/json",
				url: getContextPath() + "/device/update",
				data: JSON.stringify({
					name: $('#name_edit').val(),
					id: $('#id_edit').val(),
					series: series
